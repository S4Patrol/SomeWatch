...
import { useGetSupabaseAnimes, convertSupabaseAnimeToAniList } from "@/api/hooks/supabase-anime.hooks"
...
import { DiscoverPastSeason, DiscoverThisSeason } from "../../discover/_containers/discover-popular"
...
import { DiscoverTrendingMovies } from "../../discover/_containers/discover-trending-movies"
import { DiscoverUpcoming } from "../../discover/_containers/discover-upcoming"
            ...
            <div data-home-screen="no-entries" className="contents">
                ...
                <div className="text-center space-y-6 py-10">
                    ...
                    {!serverStatus?.settings?.library?.libraryPath && !noMediaInCollection && <>
                        ...
                        <div className="p-4 border w-fit mx-auto border-dashed rounded-xl">
                            <p>
                                Şu an izlenen bir dizi yok
                            </p>
                            <p className="text-[--muted]">
                                Başlamak için 'Şu an izlenenler' listenize dizi ekleyin
                            </p>
                        </div>
                    </>}
...
export function HomeScreenItem(props: HomeScreenItemProps) {
    ...
    if (item.type === "sitemizdekiler") {
        return (
            <>
                <SitemizdekilerCarousel libraryCollectionProps={props.libraryCollectionProps} item={item} />
            </>
        )
    }
    ...
    if (item.type === "missed-sequels") {
        return (
            <PageWrapper className="px-4">
                <DiscoverMissedSequelsSection title="Kaçırılan Devam Serileri" />
            </PageWrapper>
        )
    }
    ...
    if (item.type === "discover-sections") {
        return (
            <PageWrapper className="space-y-10 pb-10 relative z-[4] px-4">
                <div className="space-y-2 z-[5] relative">
                    <h2>Şu anda trend olanlar</h2>
                    <DiscoverTrending />
                </div>
                <RecentReleases />
                <div className="space-y-2 z-[5] relative">
                    <h2>Sezonun En İyileri</h2>
                    <DiscoverThisSeason />
                </div>
                <div className="space-y-2 z-[5] relative">
                    <h2>Geçen Sezonun En İyileri</h2>
                    <DiscoverPastSeason />
                </div>
...
function SitemizdekilerCarousel(props: { libraryCollectionProps: HandleLibraryCollectionProps, item: Models_HomeItem }) {
    const { libraryCollectionProps, item } = props
    const {} = libraryCollectionProps
    const ref = React.useRef(null)
    ...
    // Supabase'den anime verilerini çek
    const { data: supabaseAnimes, isLoading: isLoadingSupabase } = useGetSupabaseAnimes({ 
        limit: 20, 
        enabled: isInView 
    })

    // Supabase verilerini AniList formatına dönüştür ve hibrit liste oluştur
    const hybridAnimeList = React.useMemo(() => {
        if (!supabaseAnimes) return []
        return supabaseAnimes.map(anime => convertSupabaseAnimeToAniList(anime))
    }, [supabaseAnimes])
    ...
    return (
        <PageWrapper className="space-y-0 px-4" ref={ref}>
            <h2>Sitemizdekiler</h2>
            {(!isLoading && !hybridAnimeList.length && isInView) ? (
                <PageWrapper className="rounded-xl bg-gray-900 border-2 border-dashed border-orange-400 p-4 !my-4">
                    <p className="text-sm font-medium text-gray-400">
                ...
                >
                    <CarouselDotButtons className="-top-2" />
                    <CarouselContent className="px-6">
                        {hybridAnimeList.length > 0 ? hybridAnimeList.map(media => {
                            return (
                                <MediaEntryCard
                                    key={media.id}
                                    media={media}
