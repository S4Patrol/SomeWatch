...
export class VideoCoreAnime4KManager extends EventTarget {
    ...
    resize(containerWidth: number, containerHeight: number) {
        ...
        // If we still don't have video dimensions, don't shrink the canvas to 0
        if (!videoContentSize && this._boxSize.width > 0) return

        this._boxSize = { width: videoContentSize?.displayedWidth || 0, height: videoContentSize?.displayedHeight || 0 }
        
        if (this.canvas && this._boxSize.width > 0 && this._boxSize.height > 0) {
            this.canvas.width = this._boxSize.width
            this.canvas.height = this._boxSize.height
            this.canvas.style.width = this._boxSize.width + "px"
            this.canvas.style.height = this._boxSize.height + "px"
    ...
    async setOption(option: Anime4KOption, state?: {
        ...
        if (previousOption !== option && option === "off") {
            this._removeCSSEnhance()
            this.destroy()
            return
        }

        // Handle CSS-only enhancement mode (no WebGPU needed)
        if (option === "css-enhance") {
            if (previousOption !== "css-enhance") {
                // If we had a WebGPU canvas, destroy it first
                if (previousOption !== "off") this.destroy()
                this._applyCSSEnhance()
                this._onOptionChanged?.(option)
            }
            return
        }

        // If switching away from css-enhance mode, remove the CSS filter
        if (previousOption === "css-enhance") {
            this._removeCSSEnhance()
        }
        ...
        if (state) {
            ...
            if (state.isMiniPlayer || state.isPip) {
                ...
                if (previousOption !== "off" && previousOption !== "css-enhance") this.destroy()
                return
            }
        ...
        if (previousOption !== option || !this.canvas) {
            ...
            try {
                ...
                // IMPORTANT: destroy to restore video visibility on error
                this.destroy()
    ...
    // Apply CSS-only video enhancement (no WebGPU, works on any hardware)
    private _applyCSSEnhance() {
        // Contrast + sharpness + saturation boost via CSS filter
        this.videoElement.style.filter = "contrast(1.1) saturate(1.15) brightness(1.02)"
        this.videoElement.style.imageRendering = "crisp-edges"
        this.videoElement.style.opacity = "1"
        log.info("CSS enhancement applied")
    }

    private _removeCSSEnhance() {
    ...
    destroy() {
        if (this.videoElement) {
            this.videoElement.style.opacity = "1"
            this.videoElement.removeEventListener("resize", this._handleVideoResize)
        }
        // Also remove CSS enhance if it was applied
        this._removeCSSEnhance()
    ...
    private async _initialize() {
        ...
        try {
            ...
            // Listen for native video resize (e.g. metadata loaded)
            this.videoElement.addEventListener("resize", this._handleVideoResize)
    ...
    private _isOptionSelected(option: Anime4KOption): boolean {
        return option !== "off" && option !== "css-enhance"
    }
