...
import { VideoCoreFrameGen } from "@/app/(main)/_features/video-core/video-core-frame-gen"
...
import { VideoCoreFrameGenManager } from "./video-core-frame-gen-manager"
export const vc_frameGenManager = atom<VideoCoreFrameGenManager | null>(null)
            ...
            {(state?.playbackError) && (
                ...
                >
                    <div className="text-white text-center" data-vc-element="playback-error-content">
                        ...
                        {state.isAutoSwitchingProvider && state.autoSwitchCountdown !== null && (
                            <div className="mt-6 flex flex-col items-center gap-3">
                                <svg
                                    className="size-8 text-brand-500 animate-modern-spin"
                                    viewBox="0 0 24 24"
                                    fill="none"
                                ...
                                <p className="text-sm text-white/70">
                                    Sağlayıcı değiştiriliyor... ({state.autoSwitchCountdown}s)
                                </p>
                            </div>
                        )}
                    </div>
                        ...
                        <>
                            ...
                            {buffering && (
                                ...
                                >
                                    <div className="p-4">
                                        <svg
                                            className="size-16 text-brand-500 animate-modern-spin"
                                            viewBox="0 0 24 24"
                                            fill="none"
                                            xmlns="http://www.w3.org/2000/svg"
                                        >
                            ...
                            >
                                {(state.playbackInfo?.streamType === "embed") ? (
                                    <iframe
                                        src={streamUrl}
                        ...
                        >
                            ...
                            {state.loadingState && (
                                <LoadingSpinner
                                    ...
                                    spinner={<svg
                                        className="size-20 text-brand-500 animate-modern-spin"
                                        viewBox="0 0 24 24"
                                        fill="none"
                                        xmlns="http://www.w3.org/2000/svg"
                                    >
...
export interface VideoCoreProps {
    ...
    // Media info for Supabase subtitles
    mediaId?: number
    episodeNumber?: number
    onTerminateStream: () => void
    onEnded?: () => void
...
export function VideoCore(props: VideoCoreProps) {
    ...
    const [frameGenManager, setFrameGenManager] = useAtom(vc_frameGenManager)
    ...
    const handleLoadedMetadata = (e: React.SyntheticEvent<HTMLVideoElement>) => {
        ...
        if (nonLibassSubtitleTracks && nonLibassSubtitleTracks.length > 0) {
            ...
            setSubtitleManager(p => {
                ...
                // Apply episode-specific subtitle delay if available
                const episodeSubtitleDelay = (state.playbackInfo as any)?.episodeSubtitleDelay
                if (episodeSubtitleDelay !== undefined && episodeSubtitleDelay !== 0) {
                    const newSettings = {
                        ...settings,
                        subtitleDelay: episodeSubtitleDelay,
                    }
        ...
        // Initialize Frame Generation manager
        setFrameGenManager(p => {
            if (p) p.destroy()
            return new VideoCoreFrameGenManager({
                videoElement: v!,
            })
        })
    ...
    // When subtitle tracks change (e.g. Supabase subtitles load after video starts),
    // inject any NEW tracks into the running manager without restarting the video.
    const injectedSubSrcsRef = React.useRef<Set<string>>(new Set())
    React.useEffect(() => {
        // Reset injected set when the stream changes (new video)
        injectedSubSrcsRef.current = new Set()
    }, [state.playbackInfo?.streamUrl])

    React.useEffect(() => {
        const tracks = state.playbackInfo?.subtitleTracks
        if (!tracks || tracks.length === 0) return

        for (const track of tracks) {
            if (!track.src) continue
            // Skip if already injected in this session
            if (injectedSubSrcsRef.current.has(track.src)) continue

            if (track.useLibassRenderer && subtitleManager) {
                // Check if it exists in the subtitle manager (already loaded at init)
                const existsInManager = subtitleManager.getTracks().some(t => {
                    const ft = subtitleManager.getFileTrack(t.number)
                    return ft?.info.src === track.src
                })
                if (!existsInManager) {
                    log.info("Injecting new subtitle track into subtitleManager", track.label)
                    subtitleManager.addFileTrack(track)
                }
                injectedSubSrcsRef.current.add(track.src)
            } else if (!track.useLibassRenderer && mediaCaptionsManager) {
                const existsInManager = mediaCaptionsManager.getTracks().some(t => {
                    const ct = mediaCaptionsManager.getTrack(t.number)
                    return ct?.src === track.src
                })
                if (!existsInManager) {
                    log.info("Injecting new caption track into mediaCaptionsManager", track.label)
                    mediaCaptionsManager.addCaptionTrack(track as any)
                }
                injectedSubSrcsRef.current.add(track.src)
            }
        }
    }, [state.playbackInfo?.subtitleTracks?.length, subtitleManager, mediaCaptionsManager])
