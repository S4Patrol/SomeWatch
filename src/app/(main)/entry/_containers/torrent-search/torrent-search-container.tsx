...
export function TorrentSearchContainer({ type, entry }: { type: TorrentSelectionType, entry: Anime_Entry }) {
    ...
    const providerOptions = React.useMemo(() => [
        ...
        { label: "Yok", value: TORRENT_PROVIDER.NONE },
    ], [providerExtensions])
        ...
        <>
            ...
            >
                ...
                >
                    ...
                    {(type !== "download" && torrentSearchStreamEpisode) &&
                        <div className="hidden xl:block space-y-3" data-torrent-search-container-stream-episode>
                            <h4 className="!mb-4">
                                Yayını başlatmak için bir torrent seçin
                            </h4>
                            <EpisodeCard
                        ...
                        >
                            <Switch
                                ...
                                moreHelp={selectedProviderExtension?.settings?.canSmartSearch
                                    ? "Verilen parametrelere göre otomatik arama."
                                    : "Bu sağlayıcı akıllı aramayı desteklemiyor."}
                                    ...
                                    <div className="py-1" data-torrent-search-smart-search-query-input-container>
                                        <DataGridSearchInput
                                            ...
                                            placeholder={searchType === Torrent_SearchType.SMART
                                                ? `Başlığı düzenle (${entry.media?.title?.romaji})`
                                                : "Ara"}
                    ...
                    {(selectedProviderExtensionId !== "none" && selectedProviderExtensionId !== "") && Object.keys(warnings)?.map((key) => {
                        if ((warnings as any)[key]) {
                            return <Alert
                                ...
                                intent="warning"
                                description={<>
                                    {key === "extensionDoesNotSupportAdult" && "Bu sağlayıcı yetişkin içerikleri desteklemiyor"}
                                    {key === "extensionDoesNotSupportSmartSearch" && "Bu sağlayıcı akıllı aramayı desteklemiyor"}
                                    {key === "extensionDoesNotSupportBestRelease" && "Bu sağlayıcı 'en iyi sürüm' aramayı desteklemiyor"}
                                </>}
                            />
                        }
                        return null
                ...
                {(selectedProviderExtensionId !== "none" && selectedProviderExtensionId !== "") ? (
                    ...
                    <LuffyError title="Uzantı seçilmedi" />
...
function TorrentSearchTorrentStreamBatchHistory({ entry, type, debridInstantAvailability }: {
    ...
    return (
        <AppLayoutStack>
            <h5 className="text-center flex gap-2 items-center"><LuCornerLeftDown className="mt-1" /> Önceki seçim</h5>
