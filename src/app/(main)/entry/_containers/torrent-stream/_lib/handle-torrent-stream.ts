...
export function useHandleStartTorrentStream() {
    ...
    const getPlaybackType = React.useCallback((forcePlaybackMethod?: ForcePlaybackMethod) => {
        ...
        // Web Player (Denshi) mode - play in browser without MPV/VLC
        if (
            (!forcePlaybackMethod && torrentStreamingPlayback === PlaybackTorrentStreaming.WebPlayer) ||
            (forcePlaybackMethod && forcePlaybackMethod === "webplayer")
        ) {
            return "webplayer"
        }

        if (
            (!forcePlaybackMethod && __isElectronDesktop__) ||
