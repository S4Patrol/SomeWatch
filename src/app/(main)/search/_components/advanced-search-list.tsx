...
import { useSupabaseAdvancedSearch } from "@/app/(main)/search/_lib/handle-supabase-search"
...
export function AdvancedSearchList() {
    ...
    const { isLoading, data, fetchNextPage, hasNextPage, type } = useSupabaseAdvancedSearch()
    ...
    return <>
        ...
        {((data?.pages.filter(Boolean).flatMap(n => n.Page?.media).filter(Boolean) || []).length > 0 && hasNextPage) &&
            ...
            >
                ...
                <p className="text-lg font-medium">Daha fazla</p>
            </div>}
    </>
}
