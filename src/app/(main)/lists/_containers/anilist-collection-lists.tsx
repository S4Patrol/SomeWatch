...
export function AnilistCollectionLists() {
    ...
    {(!!currentList?.entries?.length && ["-", "CURRENT"].includes(selectedIndex)) && <>
        <h2>
            Güncel <span className="text-[--muted] font-medium ml-3">{currentList?.entries?.length}</span>
        </h2>
    ...
    {(!!repeatingList?.entries?.length && ["-", "REPEATING"].includes(selectedIndex)) && <>
        <h2>
            Tekrarlanan <span className="text-[--muted] font-medium ml-3">{repeatingList?.entries?.length}</span>
        </h2>
    ...
    {(!!planningList?.entries?.length && ["-", "PLANNING"].includes(selectedIndex)) && <>
        <h2>
            Planlanan <span className="text-[--muted] font-medium ml-3">{planningList?.entries?.length}</span>
        </h2>
    ...
    {(!!pausedList?.entries?.length && ["-", "PAUSED"].includes(selectedIndex)) && <>
        <h2>
            Duraklatılan <span className="text-[--muted] font-medium ml-3">{pausedList?.entries?.length}</span>
        </h2>
    ...
    {(!!completedList?.entries?.length && ["-", "COMPLETED"].includes(selectedIndex)) && <>
        <h2>
            Tamamlanan <span className="text-[--muted] font-medium ml-3">{completedList?.entries?.length}</span>
        </h2>
    ...
    {(!!droppedList?.entries?.length && ["-", "DROPPED"].includes(selectedIndex)) && <>
        <h2>
            Bırakılan <span className="text-[--muted] font-medium ml-3">{droppedList?.entries?.length}</span>
        </h2>
        <AnilistAnimeEntryList type={pageType} list={droppedList} />
    </>}

    {customLists?.map(list => {
        return (!!list.entries?.length && ["-", list.name || "Bilinmeyen"].includes(selectedIndex)) ? (
            <div
                key={list.name}
                className="space-y-6"
            >
...
export function SearchOptions({
    ...
    { value: "CURRENT", label: "İzleniyor" },
    { value: "REPEATING", label: "Tekrar izleniyor" },
    { value: "PLANNING", label: "İzlenecek" },
    { value: "PAUSED", label: "Duraklatıldı" },
    { value: "COMPLETED", label: "Tamamlandı" },
    { value: "DROPPED", label: "Bırakıldı" },
    ...(customLists || []).map(list => ({ value: list.name || "Bilinmeyen", label: list.name || "Bilinmeyen" })),
]}
                    value={selectedIndex || "-"}
            ...
            >
                <Combobox
                    ...
                    label="Tür" placeholder="Tüm türler"
                    className="w-full"
                    fieldClass="w-full"
                ...
                <Select
                    ...
                    label="Durum" placeholder="Tüm durumlar"
                    className="w-full"
                    fieldClass="w-full"
                    options={[
                        ...ADVANCED_SEARCH_STATUS,
                ...
                <Select
                    ...
                    label="Yıl" placeholder="Zamansız"
                    className="w-full"
                    fieldClass="w-full"
