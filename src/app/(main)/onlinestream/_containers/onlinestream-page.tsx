...
import { useGetSupabaseAnimeByAnilistId, SupabaseAnime } from "@/api/hooks/supabase-anime.hooks"
...
export function OnlinestreamPage({ animeEntry, animeEntryLoading, hideBackButton }: OnlinestreamPageProps) {
    ...
    // Fetch Supabase anime data for fansub subtitles
    const { data: _supabaseAnime, isLoading: isLoadingSupabaseAnime } = useGetSupabaseAnimeByAnilistId(
        mediaId ? parseInt(String(mediaId)) : 0,
        !!mediaId && !isNaN(parseInt(String(mediaId)))
    )

    // Handle case where it might be an array due to proxying
    const supabaseAnime = React.useMemo(() => {
        if (!_supabaseAnime) return null
        if (Array.isArray(_supabaseAnime)) return (_supabaseAnime as any[])[0] as SupabaseAnime
        return _supabaseAnime as SupabaseAnime
    }, [_supabaseAnime])

    // Debug: Log Supabase anime data
    React.useEffect(() => {
        if (!mediaId) {
            console.log('[FANSUB] No mediaId available')
            return
        }
        
        if (isLoadingSupabaseAnime) {
            console.log('[FANSUB] Loading Supabase anime for mediaId:', mediaId)
            return
        }
        
        if (supabaseAnime) {
            console.log('[FANSUB] Supabase anime loaded:', {
                anilistId: supabaseAnime.anilist_id,
                title: supabaseAnime.english_name || supabaseAnime.romaji_name,
                episodeCount: supabaseAnime.episodes?.length || 0,
                availableEpisodeNumbers: supabaseAnime.episodes?.map(ep => ep.number),
                fullObjectKeys: Object.keys(supabaseAnime)
            })
        } else {
            console.log('[FANSUB] No Supabase anime found for mediaId:', mediaId)
        }
    }, [supabaseAnime, mediaId, isLoadingSupabaseAnime])

    // Force refetch on mount to ensure we have the latest fansub data
    const { refetch: refetchSupabaseAnime } = useGetSupabaseAnimeByAnilistId(
        mediaId ? parseInt(String(mediaId)) : 0,
        !!mediaId && !isNaN(parseInt(String(mediaId)))
    )
    ...
    React.useEffect(() => {
        if (mediaId) {
            console.log('[FANSUB] Forcing refetch of fansub data for mediaId:', mediaId)
            refetchSupabaseAnime()
        }
    }, [mediaId, refetchSupabaseAnime])
    ...
    // Handle episode source errors (backend errors like "no source found")
    React.useEffect(() => {
        if (isErrorEpisodeSource && errorEpisodeSource && provider) {
            // Prevent infinite loop - only trigger once per provider
            if (lastErrorProviderRef.current === provider) {
                return
            }
            
            lastErrorProviderRef.current = provider
            
            const errorMessage = (errorEpisodeSource as AxiosError<{ error: string }>)?.response?.data?.error
            log.error("Episode source error", { errorMessage, provider })
            
            // Trigger auto-switching
            setIsAutoSwitchingProvider(true)
            ...
            // Switch provider after 3 seconds
            const switchTimeout = setTimeout(() => {
                const currentProviderIndex = providerExtensionOptions.findIndex((p) => p.value === provider)
                const nextProviderIndex = (currentProviderIndex + 1) % providerExtensionOptions.length
                const nextProvider = providerExtensionOptions[nextProviderIndex]
                
                if (nextProvider && nextProvider.value !== provider) {
                    setProvider(nextProvider.value)
                    setErroredServers([])
                    toast.info(`Sağlayıcı değiştiriliyor: ${nextProvider.label}`)
                } else {
                    toast.error("Başka sağlayıcı bulunamadı")
                }
                
                setIsAutoSwitchingProvider(false)
        ...
        } else if (!isErrorEpisodeSource) {
            // Reset when error is cleared
            setIsAutoSwitchingProvider(false)
            setAutoSwitchCountdown(null)
        }
    }, [isErrorEpisodeSource, errorEpisodeSource, provider, providerExtensionOptions])
    ...
    const [isAutoSwitchingProvider, setIsAutoSwitchingProvider] = React.useState(false)
    const [autoSwitchCountdown, setAutoSwitchCountdown] = React.useState<number | null>(null)
    const lastErrorProviderRef = React.useRef<string | null>(null)
    ...
    React.useEffect(() => {
        ...
        setIsAutoSwitchingProvider(false)
        setAutoSwitchCountdown(null)
        lastErrorProviderRef.current = null
    }, [currentEpisode])
            ...
            setTimeout(() => {
                ...
                if (videoSource?.server) {
                    ...
                    if (otherServers.length > 0) {
                        ...
                        toast.info("Sunucu değiştiriliyor...")
                    } else {
                        // Try next provider
                        const currentProviderIndex = providerExtensionOptions.findIndex((p) => p.value === provider)
                        const nextProviderIndex = (currentProviderIndex + 1) % providerExtensionOptions.length
    ...
    const parameters = (
        <>
            <Select
                ...
                placeholder={tUI.selectProvider}
                size="sm"
            ...
            >
                <p className="text-sm text-[--muted]">
                    {tUI.emptyTheCacheIfYouAreExperiencingIssues}
                </p>
                <Button
                    size="sm"
                    intent="alert-subtle"
        ...
        <>
            ...
            <VideoCoreInlineLayout
                ...
                leftHeaderActions={<>
                    ...
                    >
                        {dubbed ? tUI.switchToSubs : tUI.switchToDub}
                    </Button>
                ...
                rightHeaderActions={<>
                    ...
                    <IconButton
                        ...
                        title={episodeViewMode === "list" ? tUI.switchToGridView : tUI.switchToListView}
                    />
                </>}
                mediaPlayer={!provider ? (
                    <div className="flex items-center flex-col justify-center w-full h-full">
                        <LuffyError title={tUI.noProviderSelected} />
                        <div className="flex gap-2">
                            {parameters}
                        </div>
                    </div>
                ) : isEpisodeListError ? <LuffyError title={tUI.providerError}>{tUI.couldNotFetchEpisodeList}</LuffyError> : (
                    <>
                        <VideoCoreProvider id="onlinestream">
                            <div data-onlinestream-video-container className="w-full aspect-video mx-auto border rounded-lg overflow-hidden">
                                <VideoCore
                                    ...
                                    mRef={playerRef}
                                    mediaId={mediaId ? Number(mediaId) : undefined}
                                    episodeNumber={currentEpisodeNumber || undefined}
                                    state={{
                                        active: true,
                                        playbackInfo: !!url ? {
                                            id: url,
                                            ...
                                            streamType: overrideStreamType
                                                ? overrideStreamType
                                                : ((url && isHLSSrc(url)) || videoSource?.type === "m3u8") ? "hls" : ((videoSource as any)?.type === "embed" ? "embed" : "native"),
                                            subtitleTracks: (() => {
                                                // Provider subtitles
                                                const providerSubs = videoSource?.subtitles?.map((sub, index) => ({
                                                    index: index,
                                                    label: sub.language,
                                                    src: sub.url,
                                                ...
                                                // Fansub subtitles from Supabase
                                                const fansubEpisode = supabaseAnime?.episodes?.find(ep => Number(ep.number) === Number(currentEpisodeNumber))
                                                const fansubSubs = fansubEpisode?.subtitles?.map((sub, index) => ({
                                                    index: index, // Will re-index later
                                                    label: sub.label || `${sub.language} (SomeSub)`,
                                                    src: sub.url,
                                                    language: sub.language,
                                                    default: sub.language === "Turkish",
                                                    useLibassRenderer: useLibassRenderer,
                                                })) || []
                                                
                                                // Combine both, putting fansubs first
                                                const combinedSubs = [...fansubSubs, ...providerSubs]
                                                
                                                // Re-index and ensure only one default (preferring fansub Turkish)
                                                let hasDefault = combinedSubs.some(s => s.default)
                                                const allSubs = combinedSubs.map((sub, idx) => {
                                                    const isDefault = sub.default || (sub.language === "Turkish" && !hasDefault)
                                            ...
                                            episodeSubtitleDelay: episodeSource?.subtitleDelay || 0,
                                        } : null,
                                        playbackError: isErrorEpisodeSource
                                            ? (errorEpisodeSource as AxiosError<{ error: string }>)?.response?.data?.error ?? null
                                            : playbackError,
                                        loadingState: !url ? t.loadingStream : null,
                                        isAutoSwitchingProvider: isAutoSwitchingProvider,
                                        autoSwitchCountdown: autoSwitchCountdown,
                                    }}
                                    inline
                    ...
                    <AnimatePresence mode="wait" initial={false}>
                        {episodeViewMode === "list" ? (
                            ...
                            >
                                ...
                                {!!episodes?.length && <p className="text-center text-[--muted] py-2">Son</p>}
                            </motion.div>
                        ) : (
                            <EpisodePillsGrid
                                key="grid-view"
