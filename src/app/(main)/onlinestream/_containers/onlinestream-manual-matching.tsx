...
export function OnlinestreamManualMappingModal(props: OnlinestreamManualMappingModalProps) {
    ...
    return (
        <>
            <Modal
                title="Manual match"
                description="Bu animeyi sağlayıcının arama sonuçlarından biriyle eşleştirin."
                trigger={children}
...
function Content({ entry }: { entry: Anime_Entry }) {
    ...
    const confirmMatch = useConfirmationDialog({
        title: "Manual eşleştirme",
        description: "Bu animeyi arama sonucuyla eşleştirmek istediğinizden emin misiniz??",
        actionText: "Onayla",
                ...
                <AppLayoutStack>
                    <div className="text-center">
                        {!!existingMapping?.animeId ? (
                            <AppLayoutStack>
                                <p>
                                    Bulunan eşleşme: <span>{existingMapping.animeId}</span>
                                </p>
                                <Button
                            ...
                            <p className="text-[--muted] italic">Manuel eşleşme yok</p>
                        )}
                    </div>
                    ...
                    <Form schema={searchSchema} onSubmit={handleSearch}>
                        <div className="space-y-2">
                            ...
                            <Field.Switch
                                ...
                                label="Look for dubs"
                                side="right"
                                moreHelp="Yalnızca arama sonuçlarında dublajları destekleyen sağlayıcılar için geçerlidir."
                            />
