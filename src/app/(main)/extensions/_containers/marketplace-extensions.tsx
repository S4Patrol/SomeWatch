...
export function MarketplaceExtensions(props: MarketplaceExtensionsProps) {
    ...
    const fansubExtensions = filteredExtensions.filter(n => n.type === "fansub")
    ...
    return (
        <AppLayoutStack className="gap-6">
            ...
            >
                <div className="space-y-4">
                    <p className="text-sm text-[--muted]">
                        Depo JSON dosyasının URL’sini girin.
                    </p>
                    ...
                    <TextInput
                        ...
                        error={urlError}
                        placeholder="Mağazara URl'sini giriniz."
                    />
            ...
            <div className="flex items-center gap-2 flex-wrap">
                <div>
                    ...
                    <p className="text-[--muted] text-sm">
                        Depodaki eklentilere göz atın ve yükleyin.
                    </p>
                    <p className="text-[--muted] text-xs mt-1">
                        Kaynak: {marketplaceUrl === DEFAULT_MARKETPLACE_URL ?
                        <span>Resmi depo</span> :
            ...
            <div className="flex flex-wrap gap-4">
                <StaticTabs
                    ...
                    items={[
                        ...
                        {
                            name: "Fansub",
                            isCurrent: filterType === "fansub",
                            onClick: () => setFilterType("fansub"),
                            // iconType: LuBlocks,
                        },
                        {
                            name: "Eklentiler",
            ...
            {(!marketplaceExtensions && !isLoadingMarketplace) && <LuffyError>
                Mağaza eklentileri alınamadı.
            </LuffyError>}

            {(!!marketplaceExtensions && filteredExtensions.length === 0) && (
                <Card className="p-8 text-center">
                    <p className="text-[--muted]">Kriterlerinize uyan hiçbir eklenti bulunamadı..</p>
                </Card>
            )}

            {!!fansubExtensions?.length && (
                <Card className="p-4 space-y-6">
                    <div>
                        <h3 className="flex gap-3 items-center"><TbSubtask /> Fansub</h3>
                        <p className="text-[--muted] text-sm">
                            Özel fansub grupları ve çeviri ekipleri. Bu eklentiler kalıcıdır ve kaldırılamaz.
                        </p>
                    </div>
                    <div className="grid grid-cols-1 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
                        {fansubExtensions.map(extension => (
            ...
            {!!pluginExtensions?.length && (
                <Card className="p-4 space-y-6">
                    <h3 className="flex gap-3 items-center"><LuBlocks /> Eklentiler</h3>
                    <div className="grid grid-cols-1 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
            ...
            {!!animeTorrentExtensions?.length && (
                <Card className="p-4 space-y-6">
                    <h3 className="flex gap-3 items-center"><RiFolderDownloadFill />Anime torrentleri</h3>
                    <div className="grid grid-cols-1 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
            ...
            {!!onlinestreamExtensions?.length && (
                <Card className="p-4 space-y-6">
                    <h3 className="flex gap-3 items-center"><CgMediaPodcast /> Çevrimiçi yayın</h3>
                    <div className="grid grid-cols-1 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
                        {onlinestreamExtensions.map(extension => (
                            <MarketplaceExtensionCard
                                ...
                                extension={extension}
                                isInstalled={isExtensionInstalled(extension.id)}
                                disableInstall={!extension.id.toLowerCase().includes("kickassanime")}
                            />
                        ))}
                    </div>
                </Card>
            ...
            {!!customSources?.length && (
                <Card className="p-4 space-y-6">
                    <div>
                        <h3 className="flex gap-3 items-center"><MdDataSaverOn /> Özel kaynak <Popover
                            className="text-sm"
                            trigger={
                                <AiOutlineExclamationCircle className="text-[1.2rem] transition-opacity opacity-45 hover:opacity-90 cursor-pointer" />}
                        >
                            Özel kaynaklar herhangi bir yayın (streaming) özelliği sunmaz. Bunun için torrent veya çevrimiçi yayın sağlayıcıları gereklidir.
                        </Popover></h3>
                        <p className="text-[--muted] text-sm">
                            Özel kaynaklar, AniList'in sağladığının ötesinde medyaya göz atmanıza olanak tanır.
                        </p>
                    </div>
                    <div className="grid grid-cols-1 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
